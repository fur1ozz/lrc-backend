...
use App\Models\{ChampionshipPoint, ChampionshipClass, Rally, Crew, Stage, OverallResult, StageResults, CrewClassInvolvement};
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Log;

class ChampionshipPointService
{
    public function calculatePointsForRally(int $rallyId): void
    {
        DB::beginTransaction();
        ...
        try {
            ...
            if (!$rally) {
                throw new \Exception("Rally with ID {$rallyId} not found.");
            }
            ...
            if (!$lastStage) {
                throw new \Exception("No stages found for rally ID {$rallyId}.");
            }
            ...
            DB::rollBack();
            Log::error('Error calculating championship points for rally ID ' . $rallyId . ': ' . $e->getMessage());
